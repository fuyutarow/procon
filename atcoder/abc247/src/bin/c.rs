...
fn main() {
    ...
    fn SS(N: usize, memo: &mut HashMap<usize, String>) -> String {
        if N == 1 {
            String::from("1")
        } else if let Some(s) = memo.get(&N) {
            s.to_owned()
        } else {
            let s = format!("{} {} {}", SS(N - 1, memo), N, SS(N - 1, memo));
            memo.insert(N, s.clone());
            s
        }
    }

    println!("{}", SS(N, &mut memo));
}
