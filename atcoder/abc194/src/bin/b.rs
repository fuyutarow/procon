...
fn main() {
    ...
    let mut min_sofar = inf;
    for comb in AB.into_iter().permutations(2) {
        chmin!(min_sofar, comb[0].0 + comb[0].1);
        chmin!(min_sofar, max!(comb[0].0, comb[1].1));
        debug!(comb, min_sofar);
    }
