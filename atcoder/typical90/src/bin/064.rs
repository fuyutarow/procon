...
fn main() {
    input! {
        ...
        Q: usize,
        mut A: [i32; N],
        LRV: [(Usize1, Usize1, i32); Q],
    }

    let mut dA = A.iter().tuple_windows().map(|(a, b)| b - a).collect_vec();

    // let mut dA = A.windows(2).map(|w| *w[1] - *w[0]).collect_vec();

    debug!(&dA);
    for (l, r, v) in LRV {
        if (0..dA.len()).contains(&l) {
            dA[l] += v;
        }
        if (0..dA.len()).contains(&r) {
            dA[r] -= v;
        }
        debug!(&dA);
    }
